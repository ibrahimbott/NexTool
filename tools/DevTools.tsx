  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-4 p-1 bg-gray-100 rounded-lg w-fit">
        <button 
          className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${mode === 'encode' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
          onClick={() => setMode('encode')}
        >
          Encode
        </button>
        <button 
          className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${mode === 'decode' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700">Input</label>
          <TextArea 
            rows={8} 
            value={input} 
            onChange={(e) => setInput(e.target.value)}
        ...
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700">Output</label>
...
export const QrGenerator: React.FC = () => {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 items-center">
      <div className="space-y-4">
        <label className="block font-medium text-gray-700">Content (URL or Text)</label>
        <input 
          type="text" 
          value={text} 
        ...
        />
        <p className="text-sm text-gray-500">QR Code updates automatically as you type.</p>
      </div>
      <div className="flex flex-col items-center gap-4 bg-gray-50 p-6 rounded-xl">
...
export const UuidGenerator: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 text-slate-50 p-6 rounded-xl font-mono text-sm max-h-96 overflow-y-auto relative group">
        <pre>{uuids.join('\n')}</pre>
        <button 
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="bg-white p-8 rounded-2xl border shadow-sm text-center">
        <div className="text-3xl font-mono font-bold text-gray-800 break-all mb-4">{password}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {['MD5', 'SHA1', 'SHA256', 'SHA512'].map(algo => (
          <div key={algo} className="bg-gray-100 p-4 rounded-lg">
             <div className="text-xs font-bold text-gray-500 mb-1">{algo}</div>
...
export const UnixTimestamp: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
       ...
       <div className="space-y-2">
         <label className="font-bold text-sm">Convert (Date string or Epoch)</label>
         <div className="flex gap-2">
           <input className="flex-1 p-2 border rounded" value={input} onChange={(e) => setInput(e.target.value)} placeholder="e.g. 1730000000 or 2024-01-01" />
           <Button onClick={convert}>Convert</Button>
         </div>
         {converted && <div className="p-4 bg-green-50 text-green-700 font-bold rounded border border-green-200">{converted}</div>}
       </div>
    </div>
  );
};
...
export const UrlParser: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
         <input className="flex-1 p-2 border rounded" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://example.com/path?query=1" />
         <Button onClick={parse}>Parse</Button>
      </div>
      {parts && (
         <div className="bg-gray-50 rounded-lg overflow-hidden border">
           {Object.entries(parts).map(([k, v]) => (
             <div key={k} className="flex border-b last:border-0 p-3">
               <span className="w-24 font-bold text-gray-500">{k}</span>
               <span className="flex-1 font-mono text-sm text-blue-600 break-all">{v as string || '-'}</span>
             </div>
           ))}
         </div>
      )}
  ...
  return (
    <div className="space-y-6">
       <div className="grid grid-cols-3 gap-4">
          <div><label className="block text-sm mb-1">Min</label><input type="number" value={min} onChange={e => setMin(Number(e.target.value))} className="w-full p-2 border rounded" /></div>
          <div><label className="block text-sm mb-1">Max</label><input type="number" value={max} onChange={e => setMax(Number(e.target.value))} className="w-full p-2 border rounded" /></div>
          <div><label className="block text-sm mb-1">Count</label><input type="number" value={count} onChange={e => setCount(Number(e.target.value))} className="w-full p-2 border rounded" /></div>
       </div>
       <Button onClick={generate} className="w-full"><Shuffle className="w-4 h-4 mr-2 inline" /> Generate</Button>
       {result.length > 0 && (
         <div className="p-6 bg-gray-50 border rounded-xl font-mono text-2xl text-center break-all">
            {result.join(', ')}
         </div>
       )}
    </div>
